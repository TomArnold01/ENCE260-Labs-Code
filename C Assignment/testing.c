...
//This function does not work for all cases 
Float24_t float24_add(Float24_t num1, Float24_t num2)
{
    int32_t c = num1.exponent > num2.exponent? num1.exponent : num2.exponent;

    int32_t m = (int32_t)round(1.0 * num1.mantissa / pow(2, c - num1.exponent) + 
                               1.0 * num2.mantissa / pow(2, c - num2.exponent));
...
Float24_t float24_read(void) 
{
    ...
    for (int i = c+1; i < strlen(input); i++) {
        if (isdigit(input[i])) {
